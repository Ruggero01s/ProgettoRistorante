...
public class Controller implements SearchRecipe, SearchDish, Login, SaveData, DataManagement {
    ...
    private void generateAfterMeal(Set<Ingredient> consumed) {
        Set<Ingredient> temp = new HashSet<>();
        for (Ingredient regNow : model.getRegistroBeforeMeal()) {
        ...
        temp.addAll(model.getRegistroAfterMeal());
    ...
    private Set<Ingredient> compareWithRegister(Set<Ingredient> set) {
        for (Ingredient reg : model.getRegistroBeforeMeal()) {
    ...
    private void addToRegister(Set<Ingredient> set) {
        for (Ingredient ingredient : set) {
            if (model.getRegistroBeforeMeal().contains(ingredient)) {
                for (Ingredient reg : model.getRegistroBeforeMeal()) {
                    ...
                    {
                        model.getRegistroBeforeMeal().add(new Ingredient(ingredient.getName(), ingredient.getUnit(), (ingredient.getQuantity()) + reg.getQuantity()));
                        break;
                    }
                }
            } else
                model.getRegistroBeforeMeal().add(ingredient); //se non c'è già nel magazzino lo aggiungo e basta
        }
    }
    ...
    private void generateAfterMeal() {
        Set<Ingredient> registroNow = new HashSet<>(model.getRegistroBeforeMeal());

        if (!model.getBookingMap().containsKey(model.getToday())) //se oggi non c'erano prenotazioni il magazzino non è cambiato
            gui.updateWareReturnList(setToString(model.getRegistroBeforeMeal()));
        else {
            ArrayList<Booking> book = new ArrayList<>(model.getBookingMap().get(model.getToday()));
            for (Booking b : book)
    ...
    private Set<Ingredient> generateConsumedList()
	{
        ...
        if (model.getBookingMap().containsKey(model.getToday()))
		{
            ...
            for (Map.Entry<Dish, Integer> dish : allDish.entrySet())
			{
                ...
                for (Ingredient ingredient : recipe.getIngredients())
				{
                    double multi = (dish.getValue() / (double) recipe.getPortions());

                    if (consumedSet.contains(ingredient))
					{
    ...
    private Set<Ingredient> generateGroceryList(Set<Ingredient> consumedSet)
    {
        ...
        for (Ingredient consIngr: consumedSet) {
        ...
        {
            if (registroBeforeMeal.contains(consumedIngr))
            {
                for (Ingredient regIngr : registroBeforeMeal)
                {
                    if (regIngr.equals(consumedIngr))
                    {
                        double q = consumedIngr.getQuantity() - regIngr.getQuantity();
                        if (q >= 0)
                        {
    ...
    private void generateRegistroAfterMeal(Set<Ingredient> consumedList) {
        ...
        for (Ingredient consumedIngr : consumedList)
        {
                for (Ingredient beforeIngr : registroBeforeMeal)
                {
                    ...
                    {
                        double q = beforeIngr.getQuantity() - consumedIngr.getQuantity();
                        if (q > 0)
                            registroAfterMeal.add(new Ingredient(beforeIngr.getName(), beforeIngr.getUnit(), q));
                        break;
                    }
                }
        }
